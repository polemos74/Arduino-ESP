...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  if (String(topic) == "living/lamp/aux/command") {
    if (payload[1] == 'n')  {
      digitalWrite(RELAY_PIN, HIGH);
      mqttClient.publish("living/lamp/aux/feedback", 1, true, "on");
      }
    if (payload[1] == 'f'){
      digitalWrite(RELAY_PIN, LOW);
      mqttClient.publish("living/lamp/aux/feedback", 1, true, "off");
      }
}

  if (String(topic) == "living/lamp/aux/reset") {
    ESP.restart();
  }
...
void setup() {
  ...
  mqttClient.setKeepAlive(keepAliveTimeout).setCleanSession(false).setWill("clients/living/lamp/aux/lastwill", 2, true, "disconnected");
...
void buttonEvent1() {
    ...
    if (pinStatus == HIGH){
      mqttClient.publish("living/lamp/aux/pushButton", 1, true, "on");
    }
    else {
      mqttClient.publish("living/lamp/aux/pushButton", 1, true, "off");
    }
}
