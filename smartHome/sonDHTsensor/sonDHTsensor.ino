...
void setup() {
  ...
  display.showNumberDecEx(8888,64,true,4,0);
  ...
  mqttClient.setKeepAlive(60).setCleanSession(false).setWill("son/sensor/lastwill", 2, true, "disconnected");
...
//      //Serial.println(clockHour);
  if (lastHourCheck != clockHour){
    ...
    if (clockSecond%2 == 0 ){
      switch (mode) {
        case  0:
        numberToDisplay = clockHour*100 + clockMinute; 
        display.showNumberDecEx(numberToDisplay,64,true,4,0);
        break;
        
        case  1:
        numberToDisplay = int(temperature);
        data[0] = {0x00};
        data[1] = display.encodeDigit(int(numberToDisplay/10));
        data[2] = display.encodeDigit(int(numberToDisplay)-(int(numberToDisplay/10)*10));
        data[3] = 0b01100011;   // degree sign
        display.setSegments(data);
        break;
        
        case  2:
        numberToDisplay = int(humidity);
        data[0] = 0b01110110;      // H letter
        data[1] = {0x00};
        data[2] = display.encodeDigit(int(numberToDisplay/10));
        data[3] = display.encodeDigit(int(numberToDisplay)-(int(numberToDisplay/10)*10));
        display.setSegments(data);
