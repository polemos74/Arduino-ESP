// Hall lamp controller and door sensor client
...
#define REED_SWITCH D5            // Onboard push button
#define LED_ON LOW
#define LED_OFF HIGH
...
bool triggerTimerFlag = false;

char clientName[] = {"hallDoor"};
...
void onMqttConnect(bool sessionPresent) {
  uint16_t packetIdSub = mqttClient.subscribe("hall/lamp/door/command", 1);
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  if (String(topic) == "hall/lamp/door/command") {
    if (payload[1] == 'n')  {
      digitalWrite(SWITCH_PIN,HIGH);
      mqttClient.publish("hall/lamp/door/feedback", 1, true, "on");
    }
    if (payload[1] == 'f'){
      digitalWrite(SWITCH_PIN,LOW);
      mqttClient.publish("hall/lamp/door/feedback", 1, true, "off");
    }
  }
...
void setup() {
  ...
  mqttPrefix = "hall/doorController";
  ...
  attachInterrupt(REED_SWITCH,reedContact,FALLING);
  ...
  mqttClient.setKeepAlive(keepAliveTimeout).setCleanSession(false).setWill("clients/hall/alarm/lastwill", 2, true, "disconnected");
...
void event1() {
  ...
  if (digitalRead(SWITCH_PIN) == HIGH)  {
    mqttClient.publish("hall/lamp/door/pushButton", 1, true, "on");
    //Serial.println("Publishing at QoS 1");
    }
  else {
    mqttClient.publish("hall/lamp/door/pushButton", 1, true, "off");
    //Serial.println("Publishing at QoS 1");
  }
}

void event2()  {
  mqttClient.publish("hall/lamp/door/pushButton", 1, true, "auto");
  for (int a = 1 ; a < 7 ; a++) {
    digitalWrite(SWITCH_PIN,!digitalRead(SWITCH_PIN));
    delay(100);
  }
...
void checkDHTReadFlag() {
  if ( DHTReadFlag == true ) {
      ...
      uint16_t packetIdPub1 = mqttClient.publish("hall/sensor/temperature", 0, true, valueStr01);
      uint16_t packetIdPub2 = mqttClient.publish("hall/sensor/humidity", 0, true, valueStr02);
      DHTReadFlag = false;
  }
}
...
void checkReed() {
  if (reedFlag == true && triggerTimerFlag == false) {
      uint16_t packetIdPub1 = mqttClient.publish("hall/sensor/reed", 1, true, "1");
      reedFlag = false;
      triggerTimerFlag = true;
      triggerTimer.once(5,triggerTimerCallback);
  }
}

void triggerTimerCallback() {
  uint16_t packetIdPub1 = mqttClient.publish("hall/sensor/reed", 1, true, "0");
  triggerTimerFlag = false;
}
