...
char clientName[] = {"livingroomDHTSensor"};
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  if (String(topic) == "living/lamp/fairy/command") {
    if (payload[0] == '1')  {
      digitalWrite(SWITCH_PIN, HIGH);
      mqttClient.publish("living/lamp/fairy/feedback", 1, true, "on");
      }
    if (payload[0] == '0'){
      digitalWrite(SWITCH_PIN, LOW);
      mqttClient.publish("living/lamp/fairy/feedback", 1, true, "off");
      }
  }
...
void setup() {
  ...
  mqttClient.setKeepAlive(keepAliveTimeout).setCleanSession(false).setWill("clients/living/sensor/lastwill", 2, true, "disconnected");
...
void event1() {
  ...
  if (digitalRead(SWITCH_PIN) == HIGH)  {
    mqttClient.publish("living/lamp/fairy/pushButton", 1, true, "on");
    //Serial.println("Publishing at QoS 1");
    }
  else {
    mqttClient.publish("living/lamp/fairy/pushButton", 1, true, "off");
    //Serial.println("Publishing at QoS 1");
  }
}
...
void checkDHTReadFlag() {
  if ( DHTReadFlag == true ) {
      ...
      uint16_t packetIdPub1 = mqttClient.publish("living/sensor/temperature", 0, true, valueStr01);
      uint16_t packetIdPub2 = mqttClient.publish("living/sensor/humidity", 0, true, valueStr02);
      DHTReadFlag = false;
  }
}
